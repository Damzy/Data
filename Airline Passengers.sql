...
--RATING 
--LOOKING AT WHAT FACTOR CONTIBUTED THE MOST TO CUSTOMER SATISFACTION AND DISSATISFACTION  
SELECT Satisfaction, 
...
select satisfaction,
ROUND(AVG(([Departure and Arrival Time Convenience])),2), avg([Baggage Handling])
rank() over (order by Satisfaction) as rank
from airline_passenger_satisfaction$
group by Satisfaction
